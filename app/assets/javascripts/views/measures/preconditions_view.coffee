class Thorax.Views.PreconditionView extends Thorax.View
  ...
  context: -> 
    ...
    _(super).extend dataCriteria: @model.reference()
  isFirstItem: -> 
    console.log("*********** UNLESS IS FIRST ITEM *******")
    @model is @model.collection?.first()
  conjunction: -> 
    console.log("************** IN CONJUNCTION *****************")
    @model.collection?.conjunction()
  isOuterLevel: -> 
    console.log("*************** UNLESS IS OUTER LEVEL ********")
    !@model.collection?.parent.collection?


class Thorax.Views.DataCriteriaView extends Thorax.View
  template: JST['measures/data_criteria']
  context: ->
    console.log("**************** DATA CRETIERIA VIEW HAS BEEN CALLED ****************")
