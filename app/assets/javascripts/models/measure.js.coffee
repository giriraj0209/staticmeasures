...
class Thorax.Models.Submeasure extends Thorax.Model
  ...
  initialize: ->
    console.log("******************* SUBMEASURE MODEL INITIALIZE FUNCTION ******************")
    ...
    @queries = {}
  isPopulated: -> 
    console.log("********is Submeasure function is being called? *******************")
    @has 'IPP'
  fetch: (options = {}) ->
    options.data = {sub_id: @get('sub_id')} unless options.data?
    super(options)
  parse: (attrs) ->
    console.log('Im in parse function of Thorax.Models.Submeasure')
    console.log(attrs)
...
class Thorax.Collections.StatCollection extends Thorax.Collection
  #idAttribute: 'id'
  #url: -> "api/admin/caches/static_measure/#{@get('id')}"
  #url: -> "api/admin/caches/static_measure"
