...
=begin
      insurance_providers = JSON.parse(patient.extendedData['insurance_providers'])
      puts insurance_providers
      puts insurance_providers.class
      puts "11111"
      puts insurance_providers[0]["codes"]
      puts insurance_providers[0]["codes"].class
      puts "22222"
      puts v.first
      puts "33333"
      puts insurance_providers[0]['codes']['SOP'].include?(v.first)

      if (insurance_providers[0]['codes']['SOP'].include?(v.first))
        puts "inside if"
        return true
      else
        return false
...
=end
        ...
        #.map { |ip| ip['codes']['SOP'].to_s }.include?(v.first.to_s)
    end
    ...
    def self.patient_missing_problems(patient, problem)
      ...
      puts "___________________________ in patient_missing_problems ______________________________"
